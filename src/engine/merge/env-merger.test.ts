...
describe('EnvMerger', () => {
  it('creates the target env file when it does not exist', async () => {
    ...
    fileSystem.addFile(
      '/template/.env',
      ['# Base settings', '', 'API_URL=https://api.example.com', 'PORT=3000'].join(
        '\n'
      )
    )
  ...
  it('appends only missing env keys to an existing target file', async () => {
    ...
    fileSystem.addFile('/project/.env', ['API_URL=https://api.example.com', ''].join('\n'))

    await merger.merge('/template/.env', '/project/.env')

    expect(await fileSystem.readFile('/project/.env', 'utf8')).toBe(
      ['API_URL=https://api.example.com', '', 'FEATURE_FLAG=true', ' SPACED_VALUE = yes ', ''].join(
        '\n'
      )
    )
  })
  ...
  it('does not write when all source env keys already exist in the target', async () => {
    ...
    fileSystem.addFile(
      '/template/.env',
      ['# Existing keys only', 'API_URL=https://new.example.com', ' PORT = 4000 '].join('\n')
    )
    fileSystem.addFile('/project/.env', ['API_URL=https://api.example.com', 'PORT=3000', ''].join('\n'))
